...
class Property < ActiveRecord::Base
  ...
  def score
    ...
    score_map.each do |sv|
      score_value+1  unless self.send(sv)
    end

    score_value+1 if !calc_of_rentable_area == 'keine'
    score_value+1 if !plans == 'keine'
    score_value+1 if !planning_permission
    score_value+1 if !insurance
